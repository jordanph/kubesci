...
impl<'a> KubernetesContainer for PollingSidecarContainer<'a> {
  fn to_container(&self) -> Container {
    let step_check_id_map_env: String = self.steps_with_check_run_ids
    .iter()
    .map(|step_with_check_run_id| format!("{}={}", step_with_check_run_id.step.name, step_with_check_run_id.check_run_id))
    .collect::<Vec<String>>()
    .join(",");
